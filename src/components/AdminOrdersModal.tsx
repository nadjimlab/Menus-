...
import { PlacedOrder, OrderStatus, OrderItemRecord } from '../types';
...
export const AdminOrdersModal: React.FC<AdminOrdersModalProps> = ({ isOpen, onClose }) => {
  ...
  // Keep the cashier draft outside the conditionally rendered POS view.
  const [caisseTicketItems, setCaisseTicketItems] = useState<OrderItemRecord[]>([]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-3 md:p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-200">
      ...
      >
        ...
        <div className={`flex-1 overflow-hidden flex flex-col ${activeTab === 'caisse' ? '' : 'hidden'}`}>
          <CaissePOS
