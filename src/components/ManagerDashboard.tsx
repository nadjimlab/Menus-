...
export const ManagerDashboard: React.FC<ManagerDashboardProps> = ({ managerAuth }) => {
  ...
  // Navigation subtabs
  const [activeTab, setActiveTab] = useState<'orders' | 'finance' | 'expenses' | 'employees' | 'attendance' | 'security' | 'staff' | 'qrcodes' | 'products'>('orders');
  const [orderStatusFilter, setOrderStatusFilter] = useState<'active' | 'all' | OrderStatus>('active');
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#0A0A0B]">
        ...
        <div className="flex items-center gap-1 bg-[#1A1A1D] p-1 rounded-2xl border border-white/5 overflow-x-auto scrollbar-none">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap ${
              activeTab === 'orders' ? 'bg-amber-500 text-black shadow-md' : 'text-gray-300 hover:text-white'
            }`}
          >
            <Clock className="w-3.5 h-3.5" />
            <span>{isRTL ? 'الطلبات الحقيقية' : 'Commandes réelles'}</span>
            {orders.filter((order) => ['received', 'preparing', 'ready'].includes(order.status)).length > 0 && (
              <span className="w-4 h-4 rounded-full bg-red-600 text-white text-[9px] flex items-center justify-center">
                {orders.filter((order) => ['received', 'preparing', 'ready'].includes(order.status)).length}
              </span>
            )}
          </button>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-5 space-y-5">
        {/* ========================================================================= */}
        {/* SUBTAB 1: LIVE REAL ORDERS */}
        {/* ========================================================================= */}
        {activeTab === 'orders' && (
          <div className="space-y-4 max-w-6xl mx-auto">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 className="text-lg font-black text-white">{isRTL ? 'الطلبات الحقيقية' : 'Commandes réelles'}</h3>
                <p className="text-xs text-gray-400">{isRTL ? 'طلبات QR والكاشير المحفوظة فعليًا في قاعدة البيانات' : 'Commandes QR et caisse enregistrées dans la base partagée'}</p>
              </div>
              <div className="flex items-center gap-1 overflow-x-auto">
                {(['active', 'received', 'preparing', 'ready', 'completed', 'all'] as const).map((filter) => (
                  <button key={filter} onClick={() => setOrderStatusFilter(filter)} className={`px-2.5 py-1.5 rounded-lg text-[11px] font-bold whitespace-nowrap ${orderStatusFilter === filter ? 'bg-amber-500 text-black' : 'bg-[#1A1A1D] text-gray-300 border border-white/5'}`}>
                    {filter === 'active' ? (isRTL ? 'النشطة' : 'Actives') : filter === 'all' ? (isRTL ? 'الكل' : 'Toutes') : filter}
                  </button>
                ))}
              </div>
            </div>
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
              {orders
                .filter((order) => Boolean(order.id && order.createdAt && order.items?.length && Number.isFinite(order.total)))
                .filter((order) => orderStatusFilter === 'active' ? ['received', 'preparing', 'ready'].includes(order.status) : orderStatusFilter === 'all' ? true : order.status === orderStatusFilter)
                .map((order) => {
                  const nextStatus: OrderStatus | null = order.status === 'received' ? 'preparing' : order.status === 'preparing' ? 'ready' : order.status === 'ready' ? 'completed' : null;
                  return (
                    <div key={order.id} className="rounded-2xl bg-[#141416] border border-white/5 p-4 space-y-3">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="font-mono font-black text-white">{order.id}</p>
                          <p className="text-[11px] text-gray-400">{order.customerInfo.customerName || 'Client'} · {order.customerInfo.deliveryType === 'sur_place' ? `Table ${order.customerInfo.tableNumber || '?'}` : order.source || 'online'}</p>
                        </div>
                        <span className="px-2 py-1 rounded-lg bg-amber-500/15 text-amber-300 text-[11px] font-bold">{order.status}</span>
                      </div>
                      <div className="space-y-1 border-y border-white/5 py-2">
                        {order.items.map((item) => <div key={`${order.id}-${item.id}`} className="flex justify-between gap-3 text-xs"><span className="text-gray-300">{item.quantity}× {item.nameAr || item.nameFr}</span><span className="text-white font-bold">{item.totalPrice} {config.currency}</span></div>)}
                      </div>
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-black text-[#FF6321]">{order.total} {config.currency}</span>
                        {nextStatus && <button onClick={() => updateOrderStatus(order.id, nextStatus)} className="px-3 py-2 rounded-xl bg-[#FF6321] text-black text-xs font-black">{nextStatus === 'preparing' ? (isRTL ? 'بدء التحضير' : 'Préparer') : nextStatus === 'ready' ? (isRTL ? 'جاهز' : 'Prêt') : (isRTL ? 'تم التسليم' : 'Terminer')}</button>}
                      </div>
                    </div>
                  );
                })}
        ...
        {/* ========================================================================= */}
        {/* SUBTAB 2: FINANCIAL OVERVIEW & BALANCE SHEET */}
        {/* ========================================================================= */}
        {activeTab === 'finance' && (
